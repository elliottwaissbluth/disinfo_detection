...
def populate_all_visible_text(source_dir, target_dir):
    ...
    for dir_ in tqdm([source_dir / s for s in os.listdir(source_dir)]):
        ...
        if os.path.isdir(dest_dir):
            print(f'{dest_dir} has already been populated, continuing...')
            continue
        create_dir(dest_dir)
        for html in files:
            try:    
                ...
                print('Could not parse HTML at {}'.format(html))
    return
...
def tokenize_dataframe(df, medial_save=None):
    '''
    ...
    for _, row in tqdm(df.iterrows()):
        ...
        if medial_save is not None:
            if row['label'] == 'real':
                save_dir = medial_save / 'real'
            else:
                save_dir = medial_save / 'fake'

            with open(save_dir / ((str(row['index'])+'.pkl')), 'wb') as f:
                pickle.dump(final, f)

        else:
            tokenized.append(final)
    if medial_save is None:
