...
from urls.url_lists import sp_url_list
import os
...
def main(first_idx =0):
    try:
        ...
        manager_params.data_directory = Path("./datadir/sydney_powell")
...
if __name__ == '__main__':
    ...
    num_scraped = os.listdir(str(Path.cwd() / 'datadir' / 'sydney_powell' / 'sources'))
    num_scraped = [int(x.split('.')[0]) for x in num_scraped]
    num_scraped = max(num_scraped)

    first_idx = num_scraped-1
    sites_to_scrape = len(sp_url_list)
