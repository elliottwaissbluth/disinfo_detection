...
def train_hyperlinking_SVM(link_ranks):
    ...
    if not 'vectorized_links' in link_ranks.columns:
    ...
    if not 'y' in link_ranks.columns:
...
def train_logistic_regression_combined(df, col, C):
    '''
    Trains logistic regression using the combined features.
    These are already preprocessed and do not need tfidf transformation
    '''
    # Create feature array
    c = df[col].tolist()
    X = np.zeros(shape=(len(df), c[0].shape[0]))
...
def evaluate_logistic_regression_combined(model, X_test, y_test):
...
def confidence_interval_logistic_regression_combined(num_iterations, df, col='combined_features', C=1):
    '''
    Trains logistic regression classifiers on unmodified TF-IDF vectors from corpus
    '''
    results = np.zeros(shape=(4, num_iterations))
    for j in tqdm(range(num_iterations)):
        model, _, X_test, _, y_test, _ = train_logistic_regression_combined(df, col, C=C)
        accuracy, f1, precision, recall = evaluate_logistic_regression_combined(model, X_test, y_test)
        results[0, j] = accuracy
        results[1, j] = f1
        results[2, j] = precision
        results[3, j] = recall
