...
class BreadthFirstLinkSearch(BaseCommand):
    def __init__(self, url, sleep, suffix, dir = None, max_links = 100, max_no_increase = 20):
        self.url = url
        self.sleep = sleep
        self.suffix = suffix  # will be saved as <suffix>.pkl
        self.dir = dir  # Will be saved in <manager_params.source_dump_path>/dir/
        self.max_links = max_links # maximum number of links to scrape
        self.max_no_increase = max_no_increase # maximum allowable passes with no new links
        self.home_site = url

    def __repr__(self):
        return "GetTopLevelLinks({}, {})".format(self.url, self.sleep)
    ...
    def execute(
        ...
        """Calls get_website before scraping <num_links> present on the page.
        ...
        link_idx = 0
        bf_link_list = []
        num_passes_no_increase = 0 # Number of scrapes returning no new links
        visited_sites = [self.url]

        while len(list(set(bf_link_list))) < self.max_links:
            if link_idx > 0:
                # Load old link list
                
                with open(outfile, 'rb') as f:
                    bf_link_list = pickle.load(f)
                f.close()
                # Check if link index has exceeded length of list
                if link_idx >= len(bf_link_list):
                    print('Visited all urls within {}, moving to next site'.format(self.home_site))
                    print('\n{} total sites scraped from {}\n'.format(len(bf_link_list), self.home_site))
                    break

                # Set url to next link in list
                next_site =  bf_link_list[link_idx]
                # Check if site has already been visited 
                while next_site in visited_sites:
                    link_idx += 1
                    if link_idx >= len(bf_link_list):
                        print('Visited all urls within {}, moving to next site'.format(self.home_site))
                        print('\n{} total sites scraped from {}\n'.format(len(bf_link_list), self.home_site))
                ...
                self.url = next_site
                if link_idx == 1:
                    previous_len = len(list(set(bf_link_list)))
                else:
                    previous_len = len(bf_link_list)
                print('link_idx = {}, gathering from {}'.format(link_idx, self.url))
            ...
            link_urls = [links[i].get_attribute('href') for i in range(len(links))]

            logger.info(
                "BROWSER %i: logging internal links"
                % (browser_params.browser_id)
            )
            outname = str(self.suffix)
            if not self.dir:
            ...
            if link_idx > 0:
                # Concatenate and retain only new links
                combined_link_list = bf_link_list + link_urls 
                seen = set()
                full_link_list = [x for x in combined_link_list if not(x in seen or seen.add(x))]
                new_len = len(full_link_list)
                print(f'{new_len - previous_len} links added from previous scrape')
                
                # Check if any new links were added
                if previous_len == new_len:
                    num_passes_no_increase += 1
                    print('{} passes with no new links scraped'.format(num_passes_no_increase))
                else:
                    num_passes_no_increase = 0  # Reset if there are any new links
                print('\n{} total sites scraped from {}\n'.format(len(full_link_list), self.home_site))
            else:
                full_link_list = link_urls
                print('\n{} total sites scraped from {}\n'.format(len(list(set(full_link_list))), self.home_site))
            with open(outfile, "wb") as f:
                pickle.dump(full_link_list, f)
            f.close()
           
           
            # Break process if max_no_increase passes have happened with no new links
            if num_passes_no_increase == self.max_no_increase:
                print('Maximum number of passes with no new links has occurred, moving on to next site')
                break

            link_idx += 1

class GetTopLevelLinks(BaseCommand):
    def __init__(self, url, sleep, suffix, dir = None, PRINT=False):
        self.url = url
        self.sleep = sleep
        self.suffix = suffix  # will be saved as <suffix>.pkl
        self.dir = dir  # Will be saved in <manager_params.source_dump_path>/dir/
        self.print = PRINT # Whether to log a string of the list of scraped links
    ...
    def execute(
        ...
        if self.print:
            logger.info(
                "BROWSER %i: logging internal links \n%s"
                % (browser_params.browser_id, link_urls)
            )
        else:
            logger.info(
                "BROWSER %i: logging internal links"
...
class CustomSaveScreenshotCommand(BaseCommand):
    '''
    same as SaveScreenshotCommand but saves in a custom directory with custom name
    '''
...
def _custom_stitch_screenshot_parts(visit_id, browser_id, manager_params, _dir, suffix):
    ...
    for f in glob(
        ...
        _, _, index, curr_scroll = os.path.basename(f).rsplit("-", 3)
...
class CustomScreenshotFullPageCommand(BaseCommand):
    def __init__(self, suffix, dir=None):
        self.suffix = suffix
        self.dir = dir
    ...
    def execute(
        ...
        _custom_stitch_screenshot_parts(self.visit_id, self.browser_id, manager_params, 
                                        _dir=self.dir, suffix=self.suffix)
